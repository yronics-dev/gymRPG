...
// Per-stance arm rotation angles (CSS degrees around shoulder pivot)
// Pivot = top-right corner of left arm rect, top-left corner of right arm rect
const STANCE_ARMS = {
  idle:  { left: 0,   right: 0   },
  flex:  { left: 125, right: -125 },
  fight: { left: 62,  right: -62  },
...
function drawCharacter(stage, palette, muscleXP = {}, stance, equippedClothing = {}) {
  ...
  return (
    <>
      ...
      {/* Leg shading */}
      <rect x={cx - torsoW / 2 + 2} y={legTop} width="1.5" height={actualLegH} rx="0" fill="rgba(0,0,0,0.12)" />
      <rect x={cx + torsoW / 2 - legW - 2} y={legTop} width="1.5" height={actualLegH} rx="0" fill="rgba(0,0,0,0.12)" />
      ...
      <g style={{
        ...
        {/* Knuckle highlight */}
        <rect x={cx - torsoW / 2 - armW + 1} y={armTop + armH - 1} width={armW * 0.6} height="1.5" rx="0.5" fill="rgba(255,255,255,0.18)" />
        {/* Bicep bump stage 3+ */}
      ...
      {/* ─── RIGHT ARM (rotates on stance) ─────────── */}
      <g style={{
        transformOrigin: `${rightPivotX}px ${rightPivotY}px`,
        ...
        <rect x={cx + torsoW / 2 + 1} y={armTop + armH - 1} width={armW * 0.6} height="1.5" rx="0.5" fill="rgba(255,255,255,0.18)" />
        {stage >= 3 && <ellipse cx={cx + torsoW / 2 + armW / 2 - 1} cy={armTop + armH * 0.28} rx={armW * 0.55} ry={armW * 0.5} fill={s} opacity="0.5" />}
        ...
        {/* Beer / coffee in right hand (non-wave stances) */}
        {stance !== 'wave' && equippedClothing.accessory === 'acc_beer' && <>
      ...
      {/* Shirt top highlight */}
      <rect x={cx - torsoW / 2 + 2} y={torsoTop + 1} width={torsoW - 4} height="3" rx="2" fill="rgba(255,255,255,0.16)" />
      {/* V-neck collar shadow */}
      <rect x={cx - 3} y={torsoTop} width="6" height="4" rx="1" fill="rgba(0,0,0,0.18)" />
      {/* Center chest line */}
      ...
      {stage >= 3 && <>
        <ellipse cx={cx - torsoW * 0.22} cy={torsoTop + torsoH * 0.28} rx={torsoW * 0.2} ry={torsoH * 0.15} fill="rgba(0,0,0,0.10)" />
        <ellipse cx={cx + torsoW * 0.22} cy={torsoTop + torsoH * 0.28} rx={torsoW * 0.2} ry={torsoH * 0.15} fill="rgba(0,0,0,0.10)" />
        <ellipse cx={cx - torsoW * 0.22} cy={torsoTop + torsoH * 0.23} rx={torsoW * 0.11} ry={torsoH * 0.09} fill="rgba(255,255,255,0.06)" />
        <ellipse cx={cx + torsoW * 0.22} cy={torsoTop + torsoH * 0.23} rx={torsoW * 0.11} ry={torsoH * 0.09} fill="rgba(255,255,255,0.06)" />
      </>}
      {/* Armor plate stage 4+ */}
      {stage >= 4 && <>
      ...
      {/* ─── EARS ──────────────────────────────────── */}
      <rect x={cx - headW / 2 - 2} y={headTop + 5} width="3" height="4" rx="1.5" fill={s} />
      <rect x={cx + headW / 2 - 1} y={headTop + 5} width="3" height="4" rx="1.5" fill={s} />
      <rect x={cx - headW / 2 - 1} y={headTop + 6} width="1.5" height="2" rx="0.5" fill="rgba(0,0,0,0.12)" />
      <rect x={cx + headW / 2}     y={headTop + 6} width="1.5" height="2" rx="0.5" fill="rgba(0,0,0,0.12)" />

      {/* ─── HEAD ──────────────────────────────────── */}
      <rect x={cx - headW / 2} y={headTop} width={headW} height={headH} rx="4" fill={s} />
      <rect x={cx - headW / 2 + 2} y={headTop + 1} width={headW - 4} height="3" rx="2" fill="rgba(255,255,255,0.18)" />
      {/* Chin definition */}
      <rect x={cx - headW / 2 + 3} y={headTop + headH - 3} width={headW - 6} height="2" rx="1" fill="rgba(0,0,0,0.07)" />
      ...
      {(!equippedClothing.hat || equippedClothing.hat === 'hat_none') ? <>
        ...
        {/* Sideburns visible under any hat */}
      ...
      {equippedClothing.accessory === 'acc_sunglasses' ? <>
        ...
        {stage >= 4 && <>
          <rect x={cx - headW / 2 + 3} y={headTop + 5} width="4" height="4" rx="2" fill={gc} opacity="0.5" />
          <rect x={cx + headW / 2 - 7} y={headTop + 5} width="4" height="4" rx="2" fill={gc} opacity="0.5" />
        </>}
      </>}
      ...
      {stage <= 2 ? <>
        ...
        {/* Angled inner brow (determines look) */}
      ...
      {/* ─── NOSE ──────────────────────────────────── */}
      <rect x={cx - 1} y={headTop + headH / 2} width="2" height="2" rx="1" fill="rgba(0,0,0,0.14)" />
      ...
      {equippedClothing.hat === 'hat_cap' && <>
        ...
        <rect x={cx - headW / 2 + 1} y={headTop - 7} width={headW - 2} height="3"  rx="2" fill="rgba(255,255,255,0.12)" />
...
export default function CharacterSprite({
  ...
  // Aura class stays fixed to what's equipped — doesn't auto-upgrade on evolution
  ...
  const bobAnim = `pixelBob ${bobDuration} ease-in-out infinite`;
    ...
    <div className={`flex flex-col items-center gap-1 ${auraClass}`} style={{ '--aura-color': glowColor, overflow: 'visible' }}>
      <div style={{ position: 'relative', animation: bobAnim, display: 'inline-block', overflow: 'visible' }}>
        ...
        >
          ...
          {drawCharacter(stage, palette, muscleXP, stance, equippedClothing)}
        </svg>
      </div>
